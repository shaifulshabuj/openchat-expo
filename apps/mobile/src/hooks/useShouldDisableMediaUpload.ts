...
/**
 * Hook that determines if media/file upload should be disabled.
 * 
 * Disables upload for:
...
export function useShouldDisableMediaUpload(): boolean {
  return useMemo(() => {
    ...
    if (Platform.OS === 'web' && typeof window !== 'undefined' && typeof navigator !== 'undefined') {
      ...
      // Check if running on macOS (look for "macintosh" which is standard in macOS user agents)
      const isMacOS = /macintosh/.test(userAgent) && !isIOSWeb;
